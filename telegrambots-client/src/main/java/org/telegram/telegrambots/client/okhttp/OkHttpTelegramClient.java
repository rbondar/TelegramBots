...
public class OkHttpTelegramClient extends AbstractTelegramClient {
    ...
    @Override
    public CompletableFuture<Boolean> executeAsync(SetStickerSetThumbail setStickerSetThumbail) {
        try {
            assertParamNotNull(setStickerSetThumbail, "setStickerSetThumbail");
            setStickerSetThumbail.validate();

            String url = buildUrl(AddStickerToSet.PATH);

            TelegramMultipartBuilder builder = new TelegramMultipartBuilder(objectMapper);

            builder.addPart(SetStickerSetThumbail.USER_ID_FIELD, setStickerSetThumbail.getUserId())
                    .addPart(SetStickerSetThumbail.NAME_FIELD, setStickerSetThumbail.getName())
                    .addPart(SetStickerSetThumbail.THUMBNAIL_FIELD, setStickerSetThumbail.getThumbnail());

            Request httpPost = new Request.Builder().url(url).post(builder.build()).build();
            return sendRequest(setStickerSetThumbail, httpPost);
        } catch (TelegramApiException e) {
            return CompletableFuture.failedFuture(e);
        }
    }
